...
3. 可选：lighfu UnityAgent，VPM `https://lighfu.github.io/vpm/`（GitHub releases 的 `editor-v*` 标签）。
4. 用 **MA Merge Armature** / Menu Installer / Parameters 合衣服。未经要求不要 bake NDMF。
5. 优先用现场编辑器 API（`execute_csharp` / UnityAgent 工具），不要手改 `.prefab` YAML。
6. 永远不要从 MCP 调 `EditorUtility.DisplayDialog`（会卡住编辑器）。

官方 Unity 6 MCP / `com.unity.ai.assistant` 对本 2022.3 工程保持**关闭**。没有名为 UnityMCP-VCC 的活包。
