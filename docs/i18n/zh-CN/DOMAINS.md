...
| **世界产品** | 一棵 Worlds / Udon 树 | 主人覆盖层。不是这个 GitHub 仓库。可使用现有工程工具制作；工位定制 `world_*` 尚未实现，maps CLI 按需采用 |
...
| 世界 / Udon / 场景 | `skills/vrc-world` + [WORLD_PRODUCTION.md](WORLD_PRODUCTION.md)。不要走角色 `handshake.py`。Avatar 与 World 用不同 Editor |
