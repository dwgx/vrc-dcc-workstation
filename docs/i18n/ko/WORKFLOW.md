...
3. 선택: lighfu UnityAgent. VPM `https://lighfu.github.io/vpm/`（GitHub releases의 `editor-v*`）.
4. **MA Merge Armature** / Menu Installer / Parameters로 옷을 합친다. 요청 없이 NDMF를 bake하지 않는다.
5. `.prefab` YAML을 손으로 고치지 말고 라이브 에디터 API（`execute_csharp` / UnityAgent）를 쓴다.
6. MCP에서 `EditorUtility.DisplayDialog`를 호출하지 않는다（에디터가 멈춤）.

공식 Unity 6 MCP / `com.unity.ai.assistant`는 이 2022.3 프로젝트에서 **끔**. UnityMCP-VCC라는 실제 패키지는 없다.
