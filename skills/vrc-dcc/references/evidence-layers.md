...
Never promote one layer into another. `PASS` only for a layer that actually ran. Unrun = `NOT_RUN` / `MCP_REQUIRED` / `BLOCKED` / `STALE`. After an Editor mutate, mark affected **and downstream** layers `STALE_MUTATED` and recapture that layer. Do not timestamp-wash an old dump. `maps/evidence.py` tracks lease + `mutation_revision`; it does not rehash disk or prove in-world.

| Layer | Proves | Does not prove | Our usual REVIEW `gate` |
|---|---|---|---|
| `STATIC_SOURCE` | YAML / GUID literals on disk | Prefab instance, unsaved Editor dirty, NDMF output, VRChat | none (routing only) |
...
| `UDON_COMPILED` | World: UdonSharp compile + program binding | ClientSim, late join | world jobs only |
| `SDK_BUILD` | SDK window validation / exact bundle | Desktop vs PCVR vs others | `world` needs more |
| `CLIENTSIM` | World: local ClientSim interactions | Remote deserialization / late join ([ClientSim](https://creators.vrchat.com/worlds/clientsim/)) | not `world` fitted |
| `CLIENT_RUNTIME` | Named runtime: Gesture Manager, Play, Build & Test, desktop, PCVR | Other clients; upload. Build & Test ≠ Publish | `world` needs more |
| `MULTIPLAYER` | Owner/non-owner, late join after state change, owner leave | Upload | still not Publish |
