...
- [ ] スキルと MCP は **この clone** / `--mcp-config` のみ（推奨）
- [ ] 理解した：`skills/` を `~/.cursor/skills` などユーザーグローバルにコピーしない（無関係なコーディング会話が改模になる）

## Q9. この PC に無いアプリ

- [ ] Blender 無し — Blender MCP を飛ばす。ドキュメントのみでよい
- [ ] Unity 無し — Unity MCP を飛ばす。ドキュメントのみでよい
- [ ] 両方ある — ライブシーンのときだけ後で付ける
- 推奨：パスを捏造しない。例パスに合わせるために既存エディタを卸さない。

回答のあと短い計画を出し、それから実行する。所有者が確認するまで `-Apply` しない。
